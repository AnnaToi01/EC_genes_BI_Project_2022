...
tree_pic <- ggtree(tree_alrt_abayes_ufb, color="black", size=0.5, linetype=1,  right=TRUE) + # Draw tree
  ...
  geom_hilight(node=206, fill="blue", alpha=0.3, extendto=6.82) + # Highlight EC1.1 clade
  geom_hilight(node=262, fill="#B589D6", alpha=0.3, extendto=6.82) + # Highlight EC1.2 clade
...
tree_branch_length <- ggtree(tree_alrt_abayes_ufb, color="black", size=0.5, linetype=1,  right=TRUE) + 
  layout_circular() + 
  geom_tiplab(aes(color=group), size=1.5, hjust = -0.060, align=T, linesize=0.1) +  xlim(0, 15) +
  geom_hilight(node=206, fill="blue", alpha=0.3,  extendto=6.82) +
  geom_cladelabel(node=206, label="EC1.1", color="blue", offset = 1.55, offset.text = 1, fontsize = 5, barsize = 1)+
  geom_hilight(node=262, fill="#B589D6", alpha=0.3,  extendto=6.82) +
  geom_cladelabel(node=262, label="EC1.2", color="#8D45A1", offset = -2, offset.text = 0.5, fontsize = 5, barsize = 1)+
...
ggsave("tree_circ_branch_length.png",tree_pic, height = 12, width= 5)
...
save_tree_without_branchlength <- function(input_file, output_file, node_1, node_2, offset_1, offset_2) {
  ...
  tree_pic_without_branch_length <- ggtree(tree_alrt_abayes_ufb, color="black", size=0.5, linetype=1,  right=TRUE, branch.length='none') + 
    ...
    geom_point2(aes(subset=!isTip & node != root, 
                    fill=cut(ufb, c(0, 70, 90, 100))), 
                shape=21, size=10) + 
    theme_tree(legend.position=c(0.95, 0.95)) +
...
save_tree_without_branchlength("clustalo_not_trimmed_iqtree_bootstrap.treefile", "tree_circ_without_branch_length.png", 206, 262, -15.55, -19.6)

# Save the gappyout trimmed trees the same way
save_tree_without_branchlength("clustalo_gappyout_trimmed_iqtree_bootstrap.treefile", "gappyout_trimmed_tree_circ_without_branch_length.png", 366, 256, -10.5, -17.5)

# Save the gappyout trimmed trees the same way
save_tree_without_branchlength("clustalo_strict_trimmed_iqtree_bootstrap.treefile", "strict_trimmed_tree_circ_without_branch_length.png", 355, 245, -11.5, -19.5)
